...
export function MCSettingsUsersTab() {
  const utils = trpc.useUtils();
  const { data: users, isLoading, error } = trpc.admin.users.list.useQuery(
    { role: "admin" },
    { retry: false },
  );
  ...
  return (
    <section className="space-y-6">
      ...
      <div>
        <p className="text-xs tracking-[0.2em] uppercase text-muted-foreground font-light">Yönetici erişimi</p>
        <p className="text-sm text-muted-foreground mt-1">Panel girişi olan yönetici hesapları.</p>
      </div>
      ...
      <div className="bg-card border border-border/50 rounded overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-muted-foreground space-y-2">
            ...
            <p className="text-xs">Müşteriler sayfasından bir hesabı yönetici yapabilirsiniz.</p>
          </div>
        ) : (
